...
public class EnemySpawner : MonoBehaviour
{
    ...
    public bool destroyAfterDestroy = false;
    public Color failedClickColor = Color.white;
    ...
    [Header("Spawn Location Options")]
    [Tooltip("If true, enemies will spawn at a random grave inside the graveyard instead of at this spawner's position.")]
    public bool spawnAtRandomGrave = false;

    [Tooltip("Parent transform containing grave transforms as children (preferred). If null, graves with tag GraveTag will be used.")]
    public Transform graveyardParent;

    [Tooltip("Tag to locate graves if graveyardParent is not set.")]
    ...
    private int TryHandleFirstHitSpawn(Vector3 hitPoint, Collider referenceCollider)
    {
        ...
        // If configured, override spawn center with a random grave position
        if (spawnAtRandomGrave && TryGetRandomGravePosition(out Vector3 gravePos, out float graveTopY))
        {
            center = gravePos;
            topY = graveTopY;
        }
    ...
    // Try to pick a random grave position. Returns true when a valid grave was found.
    private bool TryGetRandomGravePosition(out Vector3 position, out float topY)
    {
        position = Vector3.zero;
        topY = transform.position.y;

        // Preferred: use children of graveyardParent
        if (graveyardParent != null && graveyardParent.childCount > 0)
        {
            int start = Random.Range(0, graveyardParent.childCount);
            for (int i = 0; i < graveyardParent.childCount; i++)
            {
                Transform t = graveyardParent.GetChild((start + i) % graveyardParent.childCount);
        ...
        if (!string.IsNullOrEmpty(graveTag))
        {
            GameObject[] graves = GameObject.FindGameObjectsWithTag(graveTag);
            if (graves != null && graves.Length > 0)
            {
                GameObject chosen = graves[Random.Range(0, graves.Length)];
                position = chosen.transform.position;
                topY = chosen.transform.position.y;
                return true;
            }
        }
